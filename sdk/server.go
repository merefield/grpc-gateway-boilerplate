...
import (
	...
	grpcserver "github.com/merefield/grpc-user-api/pkg/grpcserver"

	"github.com/libopenstorage/openstorage/pkg/role"
	"github.com/merefield/grpc-user-api/pkg/auth"
...
func newSdkGrpcServer(config *ServerConfig) (*sdkGrpcServer, error) {
	...
	s.userServer = &UserServer{
		server: s,
	}
	s.membershipServer = &MembershipServer{
		server: s,
	}
	s.IAMServer = &IAMServer{
		server: s,
	}
...
func (s *sdkGrpcServer) Start() error {
	...
	err := s.GrpcServer.StartWithServer(func() *grpc.Server {
		...
		api.RegisterResonateIAMServer(grpcServer, s.IAMServer)
		api.RegisterResonateUserServer(grpcServer, s.userServer)
		api.RegisterResonateMembershipServer(grpcServer, s.membershipServer)
		api.RegisterResonateCredentialsServer(grpcServer, s.credentialServer)
		...
		if s.config.Security.Role != nil {
			api.RegisterResonsateRoleServer(grpcServer, s.roleServer)
		}
		// if s.jobServer != nil {
